...
const Home = () => {
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 2fr) minmax(0, 1.5fr)', gap: '12px' }}>
                ...
                <div style={{ ...card, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
                    <div style={{ position: 'relative', width: '82px', height: '82px', borderRadius: '50%', background: 'linear-gradient(135deg, #252525, #2e2e2e)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '32px', fontWeight: '600', color: '#777', border: '1px solid #2e2e2e', boxShadow: '0 4px 12px rgba(0,0,0,0.5)', marginBottom: '18px' }}>
                        ...
                        <img src="/avatar.png" alt="Abhay Gadhvi" style={{ width: '100%', height: '100%', borderRadius: '50%', objectFit: 'cover', position: 'relative', zIndex: 1 }} onError={(e) => e.target.style.display = 'none'} />
                    </div>
                ...
                <div style={{ ...card, display: 'flex', flexDirection: 'column' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '16px', alignItems: 'center' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                            <a href="/Abhay_Gadhvi_Resume.pdf" target="_blank" rel="noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', color: '#666', textDecoration: 'none', fontSize: '13px', fontWeight: '500', transition: 'color 0.2s' }}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '12px' }}>
                {projects.map((p, i) => (
                    <Link to={`/project/${p.id}`} key={i} style={{ textDecoration: 'none', display: 'block' }}>
                        <div style={{ ...card, padding: '18px', display: 'flex', flexDirection: 'column', gap: '16px', height: '100%', transition: 'border-color 0.2s, background 0.2s', cursor: 'pointer' }}
                            onMouseEnter={e => { e.currentTarget.style.borderColor = '#444'; e.currentTarget.style.background = '#1e1e1e'; }}
                            onMouseLeave={e => { e.currentTarget.style.borderColor = '#2e2e2e'; e.currentTarget.style.background = '#191919'; }}>
                            <div style={{ height: '230px', borderRadius: '14px', border: '1px solid #272727', position: 'relative', overflow: 'hidden' }}>
                                {p.images ? (
                                    ...
                                    <>
                                        ...
                                        <img src={p.image} alt={p.title} style={{ position: 'absolute', bottom: 0, left: '50%', transform: 'translateX(-50%)', width: '85%', height: 'auto', borderRadius: '8px 8px 0 0', border: '1px solid #333', borderBottom: 'none', boxShadow: '0 10px 30px rgba(0,0,0,0.5)', zIndex: 1, objectFit: 'cover', objectPosition: 'top' }} />
                                    </>
                                ) : (
                                    <>
                                        <div style={{ position: 'absolute', inset: 0, background: '#111', backgroundImage: 'linear-gradient(#1e1e1e 1px, transparent 1px), linear-gradient(90deg, #1e1e1e 1px, transparent 1px)', backgroundSize: '30px 30px', opacity: 0.5 }} />
                                        <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                            <div style={{ width: '66px', height: '66px', borderRadius: '18px', background: '#222', border: '1px solid #333', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                                {React.cloneElement(p.icon, { size: 26, color: '#e8e8e8' })}
                                            </div>
                                        </div>
                                    </>
                                )}
                            ...
                            <div>
                                <div style={{ fontSize: '20px', color: '#e8e8e8', fontWeight: '500', marginBottom: '8px' }}>{p.title}</div>
                                <div style={{ fontSize: '14px', color: '#777', lineHeight: 1.6 }}>{p.desc}</div>
                            </div>
                        </div>
                    </Link>
                ))}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 2fr) minmax(0, 1fr) minmax(0, 2fr)', gap: '12px' }}>
